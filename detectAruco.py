...
class ARCalculator:
    def __init__(self):
        ...
        self.display_height = 80  # Height of the display box
        self.button_colors = {
    ...
    def get_button_coords(self, ui_top_left):
        ...
        # First add display box (spanning 4 buttons width)
        display_width = 4 * self.button_width + 3 * self.button_spacing
        coords.append({
            "label": "display",
            "x1": int(ui_top_left[0]),
            "y1": int(ui_top_left[1]),
            "x2": int(ui_top_left[0] + display_width),
            "y2": int(ui_top_left[1] + self.display_height),
            "state": "display"
        })
        
        # Then add buttons below the display
        for row_idx, row in enumerate(self.button_labels):
            for col_idx, label in enumerate(row):
                x1 = int(ui_top_left[0] + col_idx * (self.button_width + self.button_spacing))
    ...
    def run(self):
        while True:
            ...
            # Detect hands first
            hands, _ = self.hand_detector.findHands(frame, flipType=False)
            ...
            if ids is not None:
                for i, marker_id in enumerate(ids):
                    if marker_id in [0, 8]:
                        ...
                        # Calculate UI top-left position (shifted right from bottom right corner)
                        ...
                        # Draw UI
                        self.draw_ui(frame, button_coords)
            ...
            cv2.imshow("AR Calculator", frame)
